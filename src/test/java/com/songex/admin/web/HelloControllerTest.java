...
import com.songex.admin.config.auth.SecurityConfig;
...
@ExtendWith(SpringExtension.class) // 테스트를 진행할 때 JUnit에 내장된 실행자 외에 다른 실행자를 실행시킴 스프링 부트 테스트와 JUnit사이에 연결자 역할
...
public class HelloControllerTest {
    ...
    public void helloDto가_리턴된다() throws Exception {
        ...
        mvc.perform(
                get("/hello/dto")
                    .param("name", name) //API 테스트할 때 사용될 요청, 파라미터를 설정 값은 String만 허용, 숫자/날짜 등의 데이터도 등록할 때는 문자열로 변경해야만 가능
                    .param("amount", String.valueOf(amount)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.name", is(name))) //JSON 응답값을 필드별로 검증할 수 있는 메소드, $를 기준으로 필드명을 명시, 그래서 $.name $.amount로 검증
